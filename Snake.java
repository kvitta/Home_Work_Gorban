...
public class Snake {
    ...
    public boolean isInSnake(int x, int y) { for (Cell cell : snake) {
    ...
    public void move() {
        ...
        switch (direction) {
            case GameSnake.KEY_LEFT: x--; if (x < 0)
            ...
            case GameSnake.KEY_RIGHT: x++;
                if (x == GameSnake.CANVAS_WIDTH)
                    x = 0;
                break;
            case GameSnake.KEY_UP: y--; if (y < 0)
    ...
    public void paint(Graphics2D g) { for (Cell cell : snake) {
        cell.paint(g); }
    }
